...
**주의 — 이 표의 양쪽 끝은 믿을 수 없다. 음역 문제는 이 표로 판정하지 말 것.**

두 분석기가 서로 반대편에서 틀린다:

- **Harvest(Mode2Offline의 지표)** 는 큰 **상향** 시프트에서 틀린다. 포먼트를 보존한 채 위로
  올리면 출력이 배음 없는 사인파에 가까워지고, 그때 절반쯤의 프레임에서 배주기를 잡는다.
  저음(E2~A2) 칸이 옥타브 오류 50~70%로 찍히지만 실제 출력은 목표에 맞아 있다.
- **직접 짠 스펙트럼/NSDF 분석기**는 큰 **하향** 시프트에서 틀린다. 기음이 약해져 2·3배음을
  잡는다. 같은 방법으로 A4 칸을 재면 옥타브 오류 100%가 나오는데 Harvest로는 0.5%다.

그래서 "이 음역에서 옥타브가 틀리는가"는 **합성 출력이 아니라 검출기에 직접 물어야 한다.**
`Mode2Tests`의 `VocalPitchRange`가 E2~G5를 24k/44.1k/48k에서 검출하는지 본다 — 합성·시프트·
재분석을 거치지 않으므로 위의 착시가 끼어들 여지가 없다. 실제로 하한 100Hz 시절 이 테스트는
E2·G2에서 실패했고, 위 표는 그 원인을 가리키지 못했다.

이 표는 **중간 음역(C3~E5)의 상대 비교**에만 쓴다.
